...
import { createBannerImage, deleteBannerImage, fetchBannerImages } from '../lib/productApi'
...
function BannerManagementPanel() {
  ...
  const loadBannerItems = async () => {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : 'Unable to load banner images.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const created = await createBannerImage({
        ...form,
      ...
      setFormError(requestError instanceof Error ? requestError.message : 'Unable to upload banner image.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const handleDelete = async (item: BannerImageRecord) => {
    const confirmed = window.confirm(`Delete banner image "${item.title}"?`)
    ...
    try {
      await deleteBannerImage(item.id)
      setBannerItems((current) => current.filter((bannerItem) => bannerItem.id !== item.id))
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : 'Unable to delete banner image.')
    }
  }
    ...
    <section className="dashboard-panel dashboard-panel-wide">
      <div className="product-toolbar">
        <div>
          ...
          <p>Upload hero banner images for the home page slider and control their display order.</p>
        </div>
      ...
      {loading ? (
        <p className="product-empty">Loading banner images...</p>
      ) : sortedBannerItems.length === 0 ? (
        <p className="product-empty">No banner images uploaded yet.</p>
      ) : (
              ...
              <div className="product-form-grid">
                ...
                <label className="portal-field">
                  ...
                  <input
                    ...
                    required
                  />
                  <small>Recommended hero image size: 1920x800 or similar wide banner.</small>
                </label>
